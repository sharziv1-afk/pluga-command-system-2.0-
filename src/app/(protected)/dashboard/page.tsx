...
export default function DashboardPage() {
  ...
  const handleQuickRequestSubmit = async () => {
    ...
    try {
      ...
      await runQuickCreateMutation(
        ...
        async () => {
          ...
          const result = await supabase
            ...
            .select('id')
            .single<{ id: string }>();
          if (result.error) logSupabaseError('Dashboard quick request insert failed', result.error);
          return result;
        },
      ...
      setQuickCreateError(ambiguousMutationMessage);
    }
  };
      ...
      await runQuickCreateMutation(
        ...
        async () => {
          ...
          const result = await supabase
            ...
            .select('id')
            .single<{ id: string }>();
          if (result.error) logSupabaseError('Dashboard quick task insert failed', result.error);
          return result;
        },
      ...
      await runQuickCreateMutation(
        ...
        async () => {
          ...
          const result = await supabase
            ...
            .select('id')
            .single<{ id: string }>();
          if (result.error) logSupabaseError('Dashboard quick event insert failed', result.error);
          return result;
        },
...
function CommandBriefCard({ brief }: { brief: CommandBrief }) {
  ...
  return (
    <GlassCard
      className={`border-[var(--brand)]/18 bg-[var(--tactical-glass)] ${isAttention ? 'shadow-[0_16px_40px_rgba(255,107,2,0.12)]' : ''}`}
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <span className="block text-xs font-semibold text-[var(--color-action-on-surface)]">{formatTime(event.starts_at)}</span>
...
function OpenTaskRow({ task, assigneeName }: { task: DbTask; assigneeName: string }) {
  return (
    <Link href="/tasks" className="flex flex-col gap-3 py-3 transition hover:bg-[var(--tactical-glass)] sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0 space-y-1">
