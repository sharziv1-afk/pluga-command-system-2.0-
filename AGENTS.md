...
- Forum Daily owner mapping + scroll (2026-06-21): latest checkpoint is `62fd8fe Scroll forum daily slot selections into view` (after `1c7414c Map forum daily platoon owners to structural slots`). The UI-only mapping layer is implemented: structural platoon summary slots such as `platoon_1-summary` are enriched from active/approved owner options when a user matches both role מ״מ N and unit מחלקה N; a matched slot receives `ownerUserId`/`unitId` (`requiresOwnerMapping: false`) and that platoon report is filtered out of "existing reports" so it is not shown twice. Platoons without a matching user keep `requiresOwnerMapping: true`. Do not delete the "existing reports" fallback (legacy/unmatched reports still rely on it). Slot clicks scroll the report panel into view (`scrollIntoView({ block: 'nearest' })`) for sub-XL/single-column layouts; wide desktop does not jump. Next preferred task: Carry Forward / Rollover ("create a new day based on yesterday") — plan before code; do not start with DB population/RLS.
