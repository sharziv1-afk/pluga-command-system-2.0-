...
export const MobileHeader: React.FC<MobileHeaderProps> = ({ isSidebarOpen = false, onToggleSidebar }) => {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 flex w-full select-none items-center justify-between gap-3 border-b border-[rgba(2,1,8,0.08)] bg-white/78 px-3 py-2.5 text-right shadow-[0_10px_26px_rgba(2,1,8,0.05)] backdrop-blur-2xl xl:hidden">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            ...
            className="command-icon-button flex h-9 w-9 items-center justify-center rounded-xl border border-[rgba(2,1,8,0.10)] bg-white/70 text-[#667085] transition-all hover:border-[#FF6B02]/24 hover:text-[#FF6B02] xl:hidden"
      ...
      {isSidebarOpen && (
        <div className="w-full border-b border-[rgba(2,1,8,0.08)] bg-white/72 px-3 py-2.5 shadow-[0_10px_24px_rgba(2,1,8,0.05)] backdrop-blur-2xl xl:hidden">
            ...
            {filteredNavigationItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={onToggleSidebar}
                  className={cn(
                    'flex min-h-10 shrink-0 items-center gap-2 rounded-xl border px-3 text-[13px] font-bold transition-all duration-150',
                    isActive
