  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <CommandConfirmDialog
        open={showCompanyRefreshConfirm}
        onCancel={() => setShowCompanyRefreshConfirm(false)}
        onConfirm={confirmCompanyRefresh}
        title="רענון הדוח הפלוגתי"
        description="ערכת ידנית את הדוח הפלוגתי. רענון מהדוחות יחליף את שדות הריכוז במצב העדכני מהמ״מים (דגשי מ״פ והלו״ז הידני יישמרו). להמשיך?"
        confirmLabel="החלף ורענן"
        loading={isCompanyReportBusy}
      />

      <CommandConfirmDialog
        open={showCompanyPublishConfirm}
        onCancel={() => setShowCompanyPublishConfirm(false)}
        onConfirm={() => void publishAndCloseForum()}
        title="הפצה וסגירת פורום"
        description="פעולה זו תשמור את הנוסח הסופי ותסגור את כל דוחות היום לכלל הפלוגה. הדוחות ינעלו לקריאה בלבד. להמשיך?"
        confirmLabel="הפץ וסגור"
        loading={isCompanyReportBusy}
      />
